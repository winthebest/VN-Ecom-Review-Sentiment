...
CONFIG_PATH = BASE_DIR / "configs" / "lstm.yaml"
...
# Load data
X_train = np.load(DATA_PATH / "lstm_X_train.npy")
X_test = np.load(DATA_PATH / "lstm_X_test.npy")
y_train = np.load(DATA_PATH / "lstm_y_train.npy")
y_test = np.load(DATA_PATH / "lstm_y_test.npy")
vocab_info = np.load(DATA_PATH / "lstm_vocab_info.npy", allow_pickle=True).item()
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_last = hidden[-1]                 # lấy hidden layer cuối
...
def train_model(model, params, variant_name):
    ...
    experiment_name = f"vietnamese_sentiment_lstm_{variant_name}"
    mlflow.set_experiment(experiment_name)

    model_path = MODEL_DIR / f"lstm_{variant_name}.pt"
    ...
    with mlflow.start_run(run_name=variant_name):
        ...
        for epoch in range(params["epochs"]):
            model.train()
            total_loss, pred_train, labels_train = 0, [], []
            ...
            for X_batch, y_batch in train_loader:
                ...
                pred_train.extend(outputs.argmax(1).cpu().numpy())
                labels_train.extend(y_batch.cpu().numpy())
            
            avg_train_loss = total_loss / len(train_loader)
            train_acc = accuracy_score(labels_train,pred_train)
            train_f1 = f1_score(labels_train,pred_train, average="macro")
...
# Train all variants
for variant_name, params in config_lstm["model_variants"].items():
    print(f"Starting training for LSTM variant: {variant_name}")
    ...
    train_model(lstm_model, params, variant_name)

print("All LSTM model variants completed successfully!")
